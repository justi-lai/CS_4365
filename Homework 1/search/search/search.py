...
def depthFirstSearch(problem):
    ...
    # Item structure in Stack: (State, Path)
    stack = Stack()
...
def breadthFirstSearch(problem):
    ...
    # Item structure in Queue: (State, Path)
    queue = Queue()
...
def uniformCostSearch(problem):
    ...
    # Item structure in Priority Queue: (State, Path, Cost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    # Item structure in Priority Queue: (State, Path, g(n)) | Priority is based on lowest g(n) + h(n)
    pqueue = PriorityQueue()
    ...
    while not pqueue.isEmpty():
        ...
        if state not in expanded:
            for successor in sucessors:
                pqueue.push((successor[0], path + [successor[1]], cost + successor[2]), cost + successor[2] + heuristic(successor[0], problem))
        expanded.add(state)
